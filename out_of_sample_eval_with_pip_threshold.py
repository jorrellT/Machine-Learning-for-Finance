...
def evaluate_predictions(
    ...
    for i in range(lookback, len(df)):
        ...
        # Predict regime
        current_row['regime'] = hmm_model.predict(current_row[['log_return_z']])[0]
        regime = int(current_row['regime'].values[0])
    ...
    if visualize:
        ...
        plt.title("Prediction Probabilities and Rolling Accuracy (filtered by pip movement)")
        plt.legend()
        plt.tight_layout()
        plt.show()
